...
- Xは外部ウィジェットを使わない固定ポストカードです。`src/data/x-post.ts` に実際の投稿URL・本文・画像・日時を設定します。未設定時はカードを表示しません。返信・いいね等の数値は捏造せず、投稿を読む操作はXへ移動します。
- 配信紹介・キャラクター設定・支援内容・公式リンク一覧は `src/data/site.ts` で管理します。制作資料はGuidelinesの末尾、問い合わせはFooter直前に表示します。
